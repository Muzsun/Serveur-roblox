...
local function usingGamepad() return PADS[UserInputService:GetLastInputType()] == true end
...
pcall(function() gui.ScreenInsets = Enum.ScreenInsets.CoreUISafeInsets end) -- évite l'encoche et les bords de télé
gui.DisplayOrder = 5
...
local function adapter()
	...
	if TOUCH then s = math.max(s * 0.9, 0.6) end -- assez grand pour le doigt
	echelle.Scale = s
end
adapter()
...
local function rafraichir()
	...
	for i, outil in ordre do
		...
		b.numero.Visible = not TOUCH and not usingGamepad() -- numéros seulement avec un clavier
...
-- manette : L1 / R1 (LB / RB) pour changer d'outil, avec les icônes de la manette à côté de la barre
local function indiceManette(touche, texte, ordreAffichage)
	local cadre = Instance.new("Frame")
	cadre.BackgroundTransparency = 1
	cadre.Size = UDim2.fromOffset(44, TAILLE_BULLE)
	cadre.LayoutOrder = ordreAffichage
	cadre.Visible = false
	cadre.Parent = barre
	local image = ""
	pcall(function() image = UserInputService:GetImageForKeyCode(touche) end)
	if image ~= "" then
...
local indiceGauche = indiceManette(Enum.KeyCode.ButtonL1, "LB", -1)
local indiceDroite = indiceManette(Enum.KeyCode.ButtonR1, "RB", 1000)
local function majIndices()
	local pad = usingGamepad() and #ordre > 0
	indiceGauche.Visible = pad
	indiceDroite.Visible = pad
end
UserInputService.LastInputTypeChanged:Connect(function()
	majIndices()
	rafraichir()
end)
barre.ChildAdded:Connect(majIndices)
barre.ChildRemoved:Connect(majIndices)
majIndices()

local function changerOutil(sens)
	if #ordre == 0 then return end
	local perso = personnage()
	local actuel = 0
	for i, outil in ordre do
		if outil.Parent == perso then actuel = i end
	end
	local suivant
	if actuel == 0 then
		suivant = if sens > 0 then 1 else #ordre
	else
		suivant = (actuel - 1 + sens) % #ordre + 1
	end
	local h = humain()
	if h and h.Health > 0 then h:EquipTool(ordre[suivant]) end
end

-- touches 1 à 9 (clavier) et L1 / R1 (manette)
UserInputService.InputBegan:Connect(function(entree, occupe)
	if UserInputService:GetFocusedTextBox() or joueur:GetAttribute("MenuOuvert") then return end
	if entree.KeyCode == Enum.KeyCode.ButtonL1 then changerOutil(-1) return end
	if entree.KeyCode == Enum.KeyCode.ButtonR1 then changerOutil(1) return end
