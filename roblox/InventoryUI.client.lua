...
local OUTILS={
	{name="Ciseaux",icon="rbxassetid://98821470151953",color=Color3.fromRGB(255,120,110),items={
		...
		{id="Saisir",name="Saisir",icon="rbxassetid://120921144181867",price=1.00,max=5,
...
local function price(it) return (buyRemote and buyRemote:GetAttribute("Price_"..it.id)) or it.price end
...
local function makeRow(page,outil,item,i)
	...
	local r={row=row,stroke=st,scale=sc,item=item,outil=outil,pips=pips,lv=lv,buy=buy,face=face,faceGrad=faceGrad,
...
local function affordable()
	for _,r in rows do
		local it=r.item
		if not it.locked and level(it)<maxLevel(it) and money()>=price(it) then return true end
	end
	return false
end
...
local function refresh()
	...
	for _,r in rows do
		...
		if it.locked then
			...
			if l>=mx then buyStyle(r,"gold") r.buyText.Text="MAX"
			else
...
player.AttributeChanged:Connect(function(a) if a:sub(1,4)=="Upg_" then refresh() end end)
task.spawn(function()
