...
local CONFIG = {
	...
	INCLINAISON_AVANT = 55, -- degrés : penche les pointes vers l'avant (mets -55 si elles penchent vers toi)
	INCLINAISON_COTE = -15, -- degrés : penche les pointes vers le centre (mets 15 si elles partent vers l'extérieur)
}
...
local function construireModele()
	...
	local ACIER, FIL = Color3.fromRGB(188, 194, 203), Color3.fromRGB(242, 244, 247)
	...
	-- anneau en 12 segments (version allégée)
	local function anneau(parent, nom, cx, cz, y, a, b, d, couleur, boule)
		for i = 0, 11 do
			local t1, t2 = math.pi * i / 6, math.pi * (i + 1) / 6
			tube(parent, nom, V(cx + a * math.cos(t1), y, cz + b * math.sin(t1)),
				V(cx + a * math.cos(t2), y, cz + b * math.sin(t2)), d, couleur, boule)
		end
	end
	...
	local function branche(nom, s, y, ra, rb, angle)
		...
		anneau(poignee, "Grip", cx, cz, 0.09, ra - 0.075, rb - 0.075, 0.1, NOIR, false)
	end
...
local function construireOutil(source)
	...
	handle.CFrame = axe * CFrame.new(-1, -0.13, 0)
		...
		* CFrame.Angles(math.rad(-CONFIG.INCLINAISON_AVANT), 0, 0)
		* CFrame.Angles(0, math.rad(-CONFIG.INCLINAISON_COTE), 0)
